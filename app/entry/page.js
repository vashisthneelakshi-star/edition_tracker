...
function EditionTableRow({ edition, stateName, branch, entryDate, existingEntry, onSaved }) {
  ...
  return (
    <tr>
      ...
      <td>
        <textarea value={r.reason} onChange={e => r.setReason(e.target.value)} disabled={r.locked}
          placeholder={r.delay > 0 ? 'Required' : 'Optional'} rows={2} style={{ minWidth: 180, resize: 'vertical', fontSize: 13 }} />
      </td>
      <td>
    ...
    <div className="card" style={{ marginBottom: 12 }}>
      ...
      <div className="field-pair">
        ...
        <div>
          ...
          <textarea value={r.reason} onChange={e => r.setReason(e.target.value)} disabled={r.locked} placeholder="Max 100 words" rows={2} style={{ resize: 'vertical' }} />
        </div>
      </div>

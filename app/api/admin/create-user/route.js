...
export async function POST(req) {
  try {
    const { email, password, full_name, role, state_id, branch, requester_id } = await req.json();
    ...
    const { error: profileErr } = await supabaseAdmin.from('profiles').insert({
      ...
      full_name,
      role,
      state_id: (role === 'state_head' || role === 'edition_incharge') ? state_id : null,
      branch: role === 'edition_incharge' ? branch : null,
    });

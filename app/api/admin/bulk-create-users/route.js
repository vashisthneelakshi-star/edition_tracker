    ...
    for (const u of users) {
      try {
        ...
        const { error: profileErr } = await supabaseAdmin.from('profiles').insert({
          ...
          role: u.role,
          state_id: u.role === 'state_head' ? u.state_id : null,
        });
        if (profileErr) throw new Error(profileErr.message);

        if (u.role === 'edition_incharge' && Array.isArray(u.scopes) && u.scopes.length > 0) {
          const { error: scopeErr } = await supabaseAdmin.from('incharge_scopes').insert(
            u.scopes.map(s => ({ profile_id: userData.user.id, state_id: s.state_id, branch: s.branch }))
          );

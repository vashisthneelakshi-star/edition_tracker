...
create table if not exists profiles (
  ...
  state_id uuid references states(id),          -- used when role = state_head OR edition_incharge
  branch text,                                    -- used when role = edition_incharge (locks Branch)
  edition_id uuid references editions(id),       -- legacy, unused for edition_incharge now
  created_at timestamptz default now()
);
...
-- 5. TELEGRAM RECIPIENTS (multiple people per edition allowed)
create table if not exists telegram_links (
  id uuid primary key default gen_random_uuid(),
  edition_id uuid not null references editions(id) on delete cascade,
  chat_id text not null,
  label text,                              -- e.g. "State Head - Bikaner"
...
-- incharge can read all editions in their assigned state + branch
...
create policy "editions_incharge_read" on editions for select using (
  my_role() = 'edition_incharge' and state_id = my_state() and branch = my_branch()
);

-- ENTRIES: admin full; state_head reads entries of editions in their state;
-- incharge can insert/read/update entries for any edition in their state + branch
...
create policy "entries_incharge_read" on entries for select using (
  ...
  and edition_id in (select id from editions where state_id = my_state() and branch = my_branch())
);
...
-- TELEGRAM_LINKS: admin only manages; state_head can view own state's links

...
create table if not exists profiles (
  ...
  state_id uuid references states(id),          -- used when role = state_head
  edition_id uuid references editions(id),       -- used when role = edition_incharge
  created_at timestamptz default now()
);
...
create or replace function my_edition() returns uuid as $$
  select edition_id from profiles where id = auth.uid();
$$ language sql security definer stable;
...
-- EDITIONS: admin full access; state_head can read editions in their state; incharge can read own edition
...
create policy "editions_incharge_read" on editions for select using (
  my_role() = 'edition_incharge' and id = my_edition()
);

-- ENTRIES: admin full; state_head reads entries of editions in their state;
-- incharge can insert/read/update only their own edition's entries
...
create policy "entries_incharge_read" on entries for select using (
  my_role() = 'edition_incharge' and edition_id = my_edition()
);

...
4. `supabase/migration_lock_entries_incharge.sql` — Edition Incharge can submit
   an entry once but can no longer edit it afterward; only Admin can correct entries
5. `supabase/migration_multi_branch_incharge.sql` — lets a single Edition Incharge
   be granted access to MORE THAN ONE Branch (even across different States)
...
### Bulk Creating Users (CSV)
Admin → Users page now has a "Bulk Users Import" section at the top, same pattern
as editions: download the sample CSV, fill `full_name, email, password, role, state, branch`,
upload, preview, and confirm. Each row shows Created/Error status after import.

### Downloading Reports
The Reports page has a "Download Excel" button (top right of the period selector).
It exports a properly formatted `.xlsx` file with borders on every cell and the
Delay column colour-coded — red for late, green for early, blue for on-time —
matching what's shown on screen.

### Publishing Cycle Date & 4 AM Cutoff
The Daily Entry Form no longer lets the incharge pick any date. It automatically
shows the correct "cycle date" — since most page-release times fall between late
night and early morning, an entry made at 1:30 AM still belongs to the previous
calendar day's newspaper. The form switches to the next cycle date automatically
at 4:00 AM, after which the previous day can no longer be filled. This also keeps
every edition under the same cycle showing one consistent date (avoiding the old
problem of some editions showing one date and others showing the next).

### Entries Are Locked After Submission
An Edition Incharge can submit an entry once; after that it's locked (read-only)
for them. Only an Admin can correct it — via Reports → Daily view, where Admin
sees an extra "Correction" column with editable fields and an Update button.

### Auto-Translated Reasons in Downloads
When downloading a Daily Excel report, every "Reason" is automatically translated
to English (even if the incharge typed it in Hindi/Hinglish) before being written
to the file. This uses Google's free public translation endpoint (called directly
from the browser to avoid Vercel's server IPs being blocked) — no API key needed,
but it's an unofficial/best-effort service: if it's ever unavailable, the original
text is kept as a fallback and you'll see a small note after downloading listing
how many reasons couldn't be translated.

### Giving One Person Access to Multiple Branches
Admin → Users page now supports assigning **more than one Branch** to a single
Edition Incharge — either at creation time (add each Branch with the "+ Add"
button before creating the user) or afterward via **"Manage Branches"** next to
any existing Edition Incharge in the Users table. Their Daily Entry Form will then
show editions from every assigned Branch together. For bulk CSV import, list
multiple branches for one person separated by a semicolon in the `branch` column,
e.g. `Bikaner;Bhilwara` (all branches in a single CSV row must be in the same State
column — for branches across different States, add the person via the on-screen
form or a second CSV row followed by "Manage Branches").

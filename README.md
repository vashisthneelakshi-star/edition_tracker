...
to the file. This uses Google's free public translation endpoint — no API key
needed, but it's an unofficial/best-effort service: if it's ever unavailable or
rate-limited, the original text is kept as a fallback rather than failing the
download. For guaranteed reliability at large scale, this could later be swapped
for the paid Google Cloud Translation API (needs a billing-enabled API key).

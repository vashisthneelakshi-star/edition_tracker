...
function buildSheet(workbook, { sheetName, columns, rows, delayKey }) {
  ...
  const hasWrapColumn = columns.some(c => c.wrap);
  ...
  rows.forEach(r => {
    ...
    if (hasWrapColumn) row.height = undefined;
...
// columns: [{ header, key, width, align?: 'center'|'left', wrap?: true }]
...
// extraSheets: optional array of additional sheets: [{ sheetName, columns, rows, delayKey? }]
export async function exportStyledExcel({ filename, sheetName = 'Report', columns, rows, delayKey, extraSheets = [] }) {
  const workbook = new ExcelJS.Workbook();

  buildSheet(workbook, { sheetName, columns, rows, delayKey });
  extraSheets.forEach(s => buildSheet(workbook, s));
